...
public class NotificationServiceImpl implements NotificationService {
    ...
    @Value("${app.data.notifications.text.merchant-verified}")
    private String merchantVerifiedTemplate;

    private final NotificationApiClient notificationClient;

    @Override
    public void merchantVerified(UUID profileId, String email) {
        log.info("Service: send merchantVerified notification to email = {}", email);
        var text = String.format(merchantVerifiedTemplate, profileId);
        send(email, text);
    }
